...
public class MDB {
	...
	protected Conexion conexion;
        ...
        conexion = new Conexion("com.mysql.jdbc.Driver", "", "jdbc:mysql://localhost:3306/farmacia?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&useTimezone=true&serverTimezone=America/Mexico_City&useSSL=false", "root");
	...
	public List<ArrayList<String>> consultaAbierta(String consulta){
        ...
        public ArrayList<String> llenarCategoria(String tabla,String columna){
                int numeroColumna;
		table = tabla;
		ArrayList<String> datos = new ArrayList<>();
		try {
			executer = conexion.ObtenerConexion().createStatement();
			sqlQuery = "SELECT DISTINCT " + columna + " FROM " + table;
			dataSet = executer.executeQuery(sqlQuery);
                        numeroColumna = dataSet.findColumn(columna);
			//dataSet.beforeFirst();
			while(dataSet.next()){
			    datos.add(dataSet.getString(numeroColumna));
			}
		}
		catch (SQLException e) {
			System.out.print(e.toString() + "\n" + sqlQuery);
        ...
        public ArrayList<String> buscarID(String tabla,String columna,String condicion){
                int numeroColumna;
		table = tabla;
		ArrayList<String> datos = new ArrayList<>();
		try {
			executer = conexion.ObtenerConexion().createStatement();
			sqlQuery = "SELECT DISTINCT " + columna + " FROM " + table + " WHERE " + condicion + "LIMIT 1";
			dataSet = executer.executeQuery(sqlQuery);
                        numeroColumna = dataSet.findColumn(columna);
			dataSet.beforeFirst();
			while(dataSet.next()){
